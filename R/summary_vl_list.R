...
if (dat$param$VL_Function == "aim2") {
   namesvec_aim2 <-c("id","vl","cd4","cd4c","time")
   dat$vl_list[[at]] <- cbind(dat$pop$id[vl_ix], #Agent
                        dat$pop$V[vl_ix], #VL
                        dat$pop$CD4[vl_ix],#CD4
                        dat$pop$CD4count[vl_ix]/200, # CD4c
                        at) #Time
   colnames(dat$vl_list[[at]]) <- namesvec_aim2
}

if (dat$param$VL_Function == "aim3") {
   namesvec_aim3 <-c("id","vl","cd4","cd4c","time","muts0","muts1","muts2","muts3","muts4","muts5","drug1","drug2",
            "drug3","I","M","L","Mut1","Mut2","Mut3","Muts35","Muts12","Mut4","Mut5","drug4","Muts0")
  ...
  dat$vl_list[[at]] <- cbind(dat$pop$id[vl_ix], #Agent
    ...
    colnames(dat$vl_list[[at]]) <- namesvec_aim3
  } 
  return(dat)
}#end fxn
