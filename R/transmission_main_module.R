...
transmission_main_module <- function(dat,at)
{
  ...
  if(dat$param$model_sex!="msm"){
    ...
    temp_which <- which(hetero_sus_male == 1 &
                          dat$pop$circum[sus_id]==1)
    if(length(temp_which)>0){
  ...
  if(dat$param$transmission_model=="hill"){
    trans_probs <- transmission_hill_fxn(dat,
                                         acute_phase_status, sti_status_sus, condom_use, 
                                         msm_sus_receptive_status,
                                         msm_sus_insert_status,msm_circum_status_insert_sus,
                                         hetero_sus_female, age_vec_sus,
                                         hetero_sus_male_circum_status,
                                         V_inf, vacc_sens_sus)
  }else{
    trans_probs <- transmission_hughes_and_exp(dat,acute_phase_status,sti_status_sus,
