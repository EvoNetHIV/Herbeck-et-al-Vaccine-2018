...
// create output list of 14 elements
   ...
   if (time_0 > 12000000000) {
     printf("\n%lf: Agent %ld (2nd=%ld) entering with: D1: %lf, D2: %lf, D3: %lf, D4: %lf\n",
             time_0,Agent,SecondLineTherapyStarted,Drug1,Drug2,Drug3,Drug4);
   }
...
// Translate selected input parameters into global C variables 
 ...
 if (time_0 > 12000000) {
  ...
  KK_term = K*K*K/(V_total*V_total*V_total + K*K*K);
  ...
  do {  
    ...
    K_Fitness = K *GetAverageFitness(additive_fitness); // 1 = additive fitness, 2 = multiplicative fitness
   
    // Update carrying capacity term
    KK_term = update_carrying_capacity(K_Fitness,h);
    ...
    if (closeness_to_cutoff > 1.0) {
      if (problems <= 30) {
        ...
        if (problems == 30) {
          printf("  Problem count for agent %ld now at 30!  Silencing further warning messages\n",Agent);
        }
      }
    }
    closeness_to_cutoff = 1.0;
    ...
    update_drug_concs_pill_combos(h,Time,time_0,
    ...
    for (i5=0;i5<=alleles[5];i5++) { 
      ...
      if (do_calcs == 1) {
        ...
        growth_and_mut_terms_I = r[i1][i2][i3][i4][i5]*KK_term*((1-Num_Loci*mu)*I[i1][i2][i3][i4][i5] + mu*I_one_step);
      
       // if ( (Time > time_0 + 0.2) && (Time < time_0 + 0.2 + h)) {
       //    printf("Agent %ld (%lf):r[%1d%1d%1d%1d%1d] = %lf, IC50[4][...] = %lf, Drug[4] = %lf\n",
       //            Agent,Time,i1,i2,i3,i4,i5, r[i1][i2][i3][i4][i5], IC50[4][i1][i2][i3][i4][i5], Drug[4]);
       //  }
